-- Update function to be "deletion-safe"
...
BEGIN
  -- Check if user still exists (prevents FK error during deletion)
  SELECT EXISTS(SELECT 1 FROM "Marketing-PM-Tool".profiles WHERE id = p_user_id) INTO v_user_exists;
